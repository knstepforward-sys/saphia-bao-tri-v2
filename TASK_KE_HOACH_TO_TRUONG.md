...
| 7b | ✅ Sửa code xong. Checkbox chọn nhiều máy (+ "Chọn tất cả"/"Bỏ chọn tất cả" theo đúng danh sách đang lọc) → thanh hành động "Bố trí chạy"/"Đóng máy…" cho nhóm đã chọn. Nút "Bố trí tất cả máy chạy"/"Đóng tất cả máy" cho toàn tuần (khác 2 nút Chủ nhật). "Sao chép kế hoạch": chọn 1 ngày nguồn + nhiều ngày đích, copy nguyên trạng thái từng máy. Ô tìm máy (không phân biệt dấu) + lọc "chỉ xem máy đang đóng" (chỉ ảnh hưởng hiển thị, không đụng dữ liệu). Modal đóng máy tổng quát hoá nhận cả mảng nhiều máy (`DS_MAY_DANG_MO`). `kiem-tra.ps1` sạch cả 5 lớp (662 dòng). **Chưa `clasp push`**, chờ xác nhận riêng. | Thử: chọn vài máy → đóng hàng loạt; "Đóng tất cả máy"; sao chép 1 ngày sang nhiều ngày; tìm kiếm + lọc; Lưu; F5 kiểm tra |
