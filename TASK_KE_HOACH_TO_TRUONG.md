...
| 7a | ✅ Sửa code xong. Modal đóng máy (áp cả tuần / một ngày / một ngày-một ca), nút "Bố trí lại", nút "Lưu kế hoạch tuần" gộp 1 lượt gọi `luuKeHoachTuan`. State client (`NGOAI_LE`) chỉ đọc từ server lúc tải tuần, mọi thao tác đóng/mở chỉ sửa bộ nhớ, ghi lên server khi bấm Lưu. Cảnh báo khi đổi tuần mà có thay đổi chưa lưu. Thêm `lyDoDongMay` vào `getToTruongBootstrap`. `kiem-tra.ps1` sạch cả 5 lớp (419 dòng). **Chưa `clasp push`**, chờ xác nhận riêng. | Thử luồng thật: đóng 1 máy cả tuần, đóng 1 máy khác chỉ 1 ngày, đóng 1 máy chỉ 1 ngày+1 ca, Lưu, F5 lại đúng dữ liệu; thử "Bố trí lại" |
