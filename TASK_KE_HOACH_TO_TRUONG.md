...
| 6 | ✅ Sửa code xong. `ToTruong.html` đầy đủ: màn hình khai giờ làm việc lần đầu (nếu `lich===null`) → màn hình chính (chọn tuần + thống kê nhanh + danh sách máy, mỗi máy hiện "Bố trí chạy" hoặc liệt kê ngoại lệ đóng máy theo ngày/ca). Dùng `<input type="time">`, `google.script.run` gọi `getToTruongBootstrap`/`luuLichLamViec`/`layKeHoachTuan` đã có từ bước 3-5. Chưa có nút sửa từng máy / thao tác hàng loạt (bước 7). `kiem-tra.ps1` sạch cả 5 lớp (HTML 228 dòng, không lỗi). **Chưa `clasp push`**, chờ xác nhận riêng. | Mở link Test deployment, xem giao diện thật trên điện thoại — thử cả 2 nhánh: token của tổ CHƯA có lịch (màn hình khai giờ) và token của DET đã có lịch/kế hoạch test cũ (màn hình chính, thấy đúng dữ liệu 4T-12 đóng máy tuần 21/09 từ lúc test bước 5) |
