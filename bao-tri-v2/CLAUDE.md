...
| **Quy tắc xác thực dữ liệu đặt nhầm cột** | **Âm thầm huỷ lệnh ghi của script.** Apps Script gom lệnh ghi rồi mới đẩy xuống Sheet: `setValues` trả về êm ru, hàm báo "đã ghi 748 dòng", Google từ chối lúc đẩy xuống, không một dòng thông báo. Ngày 12/09/2026 một dropdown `Loai_Phieu` tạo tay (bản cũ 4 giá trị, thiếu `HO_TRO`) nằm trên cột **AF** — tức `KPI_Ap_Dung`, lệch 4 cột so với `Loai_Phieu` ở **AB** — làm menu 🎯 nhiều ngày liền không ghi được ô nào | `setupSystem` gọi `goQuyTacXacThuc_()` gỡ sạch quy tắc trên 12 cột mã tự ghi của `Su_Co` và `Luu_Tru`. Mục 🩺 soi và chỉ mặt cột còn vướng. Đừng tạo dropdown tay trên `Su_Co` |
| **Đọc lại ngay sau khi ghi, trong cùng một lượt** | Đọc phải trạng thái **trước** lệnh ghi. `donPhieuCuHangThang()` gọi tính KPI rồi gọi dọn phiếu liền, phần dọn thấy 520 phiếu "chưa tính KPI" ngay dưới dòng báo vừa ghi xong 748 dòng — vòng lặp không lối ra, bấm 🎯 trước bao nhiêu lần cũng vô ích | `SpreadsheetApp.flush()` ngay sau `setValues`, trước khi hàm sau đọc lại |
